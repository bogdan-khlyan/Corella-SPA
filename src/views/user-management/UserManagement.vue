      ...
      <el-table v-loading="loading" :data="users">
        <el-table-column width="60">
          <template #default="scope">
            <base-user-avatar :avatar="scope.row.login" />
          </template>
        </el-table-column>

        <el-table-column prop="login" label="Login" min-width="200" />
        ...
        <el-table-column prop="isBan" label="Ban" width="200px">
          <template #default="scope">
            <el-switch
              ...
              @change="changeBanSwitch(scope.row.id, scope.row.isBanned)"
            />
          </template>
        </el-table-column>
...
<script>
import UserManagementModal from '@/views/user-management/UserManagementPopup'
...
export default {
  ...
  methods: {
    changeBanSwitch(userId, isBanned) {
      this.$api.users.patchUser(userId, {
        isBanned,
      })
    },
    ...
    async getUsers() {
      this.loading = true
      const users = await this.$api.users.getUsers(this.pagination)
